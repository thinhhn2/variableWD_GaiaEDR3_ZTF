...
par(mar=c(6,6,5,3)+.1)
plot(timeG,G,ylim=c(max(G)+err[G==max(G)],min(G)-err[G==min(G)]),col="red",xlab="",ylab="",cex=1.6,cex.lab=2.5,cex.axis=2,cex.main=2.5,pch=16,main= sprintf("%s ZTF Full Light Curve\n in r band",n))
...
png(file=filename.lc,width=700,height=520)
par(mar=c(6,6,5,3)+.1)
plot(timeG,G,ylim=c(max(G)+err[G==max(G)],min(G)-err[G==min(G)]),col="red",xlab="",ylab="",cex=1.6,cex.lab=2.5,cex.axis=2,cex.main=2.5,pch=16,main= sprintf("%s ZTF Light Curve\n in r band",n))
title(xlab="JD - 2400000.5",cex.lab=2.5,line=4)
title(ylab="ZTF g mag",cex.lab=2.5,line=3.5)
arrows(timeG, G-err, timeG, G+err, length=0.05, angle=90, code=3, col="red")
box(lwd=2)
...
filename.pc = sprintf("%s Phase Curve r band.png",n)
png(file=filename.pc,width=700,height=520)
par(mar=c(6,6,5,3)+.1)
plot(phase,G,ylim=c(max(G.1)+err[G.1==max(G.1)],min(G.1)-err[G.1==min(G.1)]),col="red",cex.main=2.5,pch=16,main=sprintf("%s ZTF Folded Curve \nin r Band",n),ylab="",xlab="",cex=1.6,cex.lab=2.5,cex.axis=2)
